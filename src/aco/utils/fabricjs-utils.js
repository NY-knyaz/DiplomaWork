...
import NormalizeUtils from './normalize-utils';
...
class FabricjsUtils {
  ...
  static makeGABestRoute(x1, y1, x2, y2) {
    return new fabric.Line([x1, y1, x2, y2], {
      stroke: 'red',
      strokeWidth: 2,
      selectable: false,
  ...
  static makeEdges(environment) {
    ...
    for (var i = 0; i < environment.getNumberOfNodes(); i++) {
      for (var j = i + 1; j < environment.getNumberOfNodes(); j++) {
        ...
        let edge = FabricjsUtils.makeLine(
          n1.left - 5,
          n1.top - 5,
          n2.left - 5,
          n2.top - 5
    ...
    return new fabric.Group(items, {
      selectable: false,
      evented: false,
      layer: FabricjsUtils.LAYER.EDGE,
    });
  }
